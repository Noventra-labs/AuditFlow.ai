...
export default function DashboardView({ setActiveView }: { setActiveView: (view: string) => void }) {
  return (
    <>

{/* Fixed Side Navigation (Spine) */}
...
<button className="text-[#62df7d] bg-[#1c2026] scale-110 p-2 rounded transition-all" onClick={() => setActiveView('dashboard')}><span className="material-symbols-outlined">grid_view</span></button>
<button className="text-[#dfe2eb]/40 hover:text-[#62df7d] hover:bg-[#1c2026]/50 transition-all p-2 rounded" onClick={() => setActiveView('forecast')}><span className="material-symbols-outlined">monitoring</span></button>
<button className="text-[#dfe2eb]/40 hover:text-[#62df7d] hover:bg-[#1c2026]/50 transition-all p-2 rounded" onClick={() => setActiveView('invoices')}><span className="material-symbols-outlined">receipt_long</span></button>
<button className="text-[#dfe2eb]/40 hover:text-[#62df7d] hover:bg-[#1c2026]/50 transition-all p-2 rounded" onClick={() => setActiveView('tax_compliance')}><span className="material-symbols-outlined">account_balance</span></button>
<button className="text-[#dfe2eb]/40 hover:text-[#62df7d] hover:bg-[#1c2026]/50 transition-all p-2 rounded" onClick={() => setActiveView('reports')}><span className="material-symbols-outlined">description</span></button>
<button className="text-[#dfe2eb]/40 hover:text-[#62df7d] hover:bg-[#1c2026]/50 transition-all p-2 rounded" onClick={() => setActiveView('reconciliation')}><span className="material-symbols-outlined">verified_user</span></button>
...
<button className="text-[#62df7d] font-semibold text-sm flex items-center gap-3 px-3 py-2 bg-[#1c2026] rounded-md text-left">
<span className="material-symbols-outlined text-sm">corporate_fare</span> Entity Selector
            </button>
<button className="text-[#dfe2eb]/60 hover:bg-[#1c2026] hover:text-[#dfe2eb] transition-colors text-sm flex items-center gap-3 px-3 py-2 rounded-md text-left">
...
{/* Top Nav Bar */}
<header className="h-16 flex items-center justify-between px-8 bg-[#10141a]/80 backdrop-blur-xl sticky top-0 z-30">
...
<input className="bg-transparent border-none focus:ring-0 text-sm w-full placeholder:text-on-surface-variant/50" placeholder="Search transactions, accounts, or entities..." type="text"/>
</div>
...
<img alt="User" data-alt="minimalist professional portrait of a financial executive with clean lighting and corporate background" src="https://lh3.googleusercontent.com/aida-public/AB6AXuDJoc3dbfORt1gHTuqacgTtIuR_jGfL0qjuEgjahQLRW-LhWX0wAgjPbHI4cc5alDnjJoJUamni2oBtTOI9PKKuo_DGdIkXoE3EH-BuMtXH-a2vvZWgRFwhFxymUqFx_mbykv-uiI1oEBknS2oQrIsmg2GkN6IQgfEamYKptRk9YPGXOItsir-yp_q-AywsDlkENzYxpATqY1PAJAfRS8WzlhSDX_ILDqQ4zDeCb49FDuILCCQpnil7NmUzgKjk4Btvaj32M2E1ipav"/>
</div>
</div>
</header>
{/* Sticky Alert Banner */}
...
<button className="text-[10px] font-bold uppercase tracking-widest text-amber-400 hover:text-amber-200 transition-colors">Resolve Now</button>
</div>
...
{/* Drawing a decorative abstract sparkline with SVG */}
...
<img alt="Quickbooks" className="w-6 h-6 grayscale brightness-150" data-alt="abstract geometric logo icon for accounting software integration" src="https://lh3.googleusercontent.com/aida-public/AB6AXuDqzCBpMyfctWQGsRFx6v9BI-pNxd78-9CEzSZfUqWVs8mIL6PiU-DTVuFpA7BbXWveBtpzb76nJC9z7vFWoqlm7xE1SC8XdI-OkJiugU7LxkqapFIRLLlPdg1U0ngPRZEIZ4gqxmIYkrlR3ahLqYL1lZRFjg6lbOKOKv8bBXrV9ZVkSmwSAa-sQzC_egFprWnc_NwP3vcb7gkstzbLMysYWUWCikUS2svEGkAbEFokYTVrEl-x7nC1gb87QcOXJ5wE9EezB_45LcLc"/>
</div>
<div className="w-10 h-10 rounded bg-surface-container-highest flex items-center justify-center">
<img alt="Stripe" className="w-6 h-6 grayscale brightness-150" data-alt="abstract geometric logo icon for payment processor integration" src="https://lh3.googleusercontent.com/aida-public/AB6AXuBAQ2gIZKQGo9JWhcLHe_fsWdcVVCkub6Rw9dPDot7k4EU_4FpwGUKcFo660lmDQVxqCP9PbG2lAubOmX-fSJgo-LCWWY7QYreqADJEOqiSQpB2qFV6qLYN9CAWg8sPDBCmPlfsQRjPATUS_b_4CCwU_2X3BkUjfFKoBh0lPYDOMjXV9vRGN01PuyroFS_5uFBdzty4_AGZMWXzSLXpw-vS894_YgR4R_RUVWI-CcU08T4DBz2C4x4c3GfW4oWNCIz-c52T2NfEoX2L"/>
</div>
